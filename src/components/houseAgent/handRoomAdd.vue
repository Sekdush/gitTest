          ...
          <Form :model="formItem" :label-width="100">
            ...
            <FormItem label="单价（元/㎡）">
                <Row>
                    ...
                    <Col span="2" style="text-align: right;padding-right:10px">户型</Col>
                    <Col span="7">
                    <Row>
                        <Col span="8">
            ...
            <FormItem label="楼层">
                <Row>
                    <Col span="7">
                        <Input v-model="formItem.config.floorNum" placeholder="楼层" />
                    </Col>
                    <Col span="2" style="text-align: right;padding-right:10px">车位</Col>
                    <Col span="7">
                        <RadioGroup v-model="formItem.config.carnum">
                ...
                <Row>
                    <Col span="7">
                        <CheckboxGroup v-model="formItem.config.tag">
                            ...
                            <Checkbox label="满十年"></Checkbox>
                            <Checkbox label="全新"></Checkbox>
                        </CheckboxGroup>
                    </Col>
  ...
  mounted() {
    ...
    if (this.$route.query.id) {
      ...
      if (sessionStorage.getItem("adminName")) {
        this.formItem.config.name = sessionStorage.getItem("adminName");
      }
      if (sessionStorage.getItem("adminPhone")) {
        this.formItem.config.phone = sessionStorage.getItem("adminPhone");
      }
    }
  },
  methods: {
    getMa() {
      ...
      $.ajax({
        ...
        success(res) {
          if (res.code) {
            that.formItem.config.village = res.params.result[0].title;
            that.formItem.config.name = res.params.result[0].config.name;
            that.formItem.config.phone = res.params.result[0].config.phone;
            that.formItem.minPrice = res.params.result[0].minPrice;
            that.formItem.note = res.params.result[0].title + res.params.result[0].config.lnum +'幢'+res.params.result[0].config.mnum+'单元'+res.params.result[0].config.pnum+'室'
          } else {
    ...
    submitForm() {
      var that = this;
      $.ajax({
        url: sessionStorage.getItem("API") + "exh/add",
        type: "POST",
        data: {
          ...
          config: JSON.stringify(this.formItem.config),
