...
export default {
  ...
  data() {
    return {
      ...
      data: {
        ...
        richText: "",
        access: "",
        type: localStorage.getItem("miniId")
      },
  ...
  methods: {
    submitForm(){
      ...
      var that = this;
      $.ajax({
        url:sessionStorage.getItem('API') + 'art/add',
        type:'POST',
        data:this.data,
        dataType:'json',
        success(res){
    ...
    getModelCover() {
      this.data.cover  = JSON.parse(sessionStorage.getItem("imgUrls"))[0].host + JSON.parse(sessionStorage.getItem("imgUrls"))[0].path;
